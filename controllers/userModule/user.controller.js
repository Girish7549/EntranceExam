...
export const createUser = async (req, res) => {
  const { first_name, last_name, email, password, phone, status } = req.body;

  // Simple validation
  if (!first_name || !last_name || !email || !password || !phone || !status) {
    return res.status(400).json({ error: 'All fields are required' });
  }

  try {
    const result = await createUserModel({ first_name, last_name, email, password, phone, status });
